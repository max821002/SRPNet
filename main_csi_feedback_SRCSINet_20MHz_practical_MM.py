...
for idx_ant in range(len(D_ant_set)):
    D_ant = D_ant_set[idx_ant]
...
#    
      mat_dl = mat73.loadmat(filedir+'H_dl_Qp='+str(Qp)+'_Qa='+str(Qa)+'_'+str(sc)+'KHz.mat')
    
      h_dl_RB = mat_dl['H_RB_channel'][:,:,:No_RB]
      h_ul_RB = mat_dl['H_RB_UL_channel'][:,:,:No_RB]
      f_dl_RB = mat_dl['H_RB_precoder'][:,:,:No_RB]
      f_dl_SB = mat_dl['H_SB_precoder'][:,::D_ant,0:24:No_RB_per_SB//4]
    ...
    #    valset = data_set(torch.Tensor(input_val),torch.Tensor(si_val), torch.Tensor(label_val))
        ...
        for idx_cr in range(len(CR_set)):
            ...
            if idx_model == 0:
              if SC_spacing == 15:
                  Upsampler = SRCsiNet_BD([D_ant,No_RB_per_SB]).to(device)
              else:
                  Upsampler = SRCsiNet_BD([D_ant,No_RB_per_SB]).to(device)
    ...
    #            input2 = input2.view([BATCH_SIZE*2*8,No_RB*12])
                optimizer.zero_grad()
                out, a, b, c, d = Upsampler(input,4//No_RB_per_SB)
    ...
    #              out = out.view([BATCH_SIZE,2,8,No_RB*12])
        ...
        #        test_loss[epoch] = 10*torch.log10(epoch_loss/TEST_SIZE)
                ...
                if val_loss[epoch] < best_loss:
                  print("Saving...")
                  torch.save(Upsampler, "model_"+model_name+"_20MHz_MM_practrical_Qp_Qa="+str(Qp)+"_"+str(Qa)+"_DS="+str(No_RB_per_SB)+"_Dant"+str(D_ant)+"_"+str(sc)+'KHz_pth')               
                  count = 0
                else:
                  count += 1
                if epoch == 0:
