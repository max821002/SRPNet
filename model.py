...
class ULCSISwitch_norm_multilayer(nn.Module):
    def __init__(self):
        super(ULCSISwitch_norm_multilayer, self).__init__()
        self.dense1 = nn.Linear(96,48)
        self.dense2 = nn.Linear(48,24)
        self.dense3 = nn.Linear(24,1)
    def forward(self, x, slope):
        PDP = torch.squeeze(torch.sum(torch.abs(torch.fft.ifft(x[:,0,:,:]+1j*x[:,1,:,:]))**2,-2))
        max_val, _ = torch.max(PDP,-1,keepdim=True)
        PDP = PDP/max_val
        out = self.dense1(PDP)
        out = self.dense2(out)
        out = self.dense3(out)
        return nn.Sigmoid()(out*slope)
class ULCSISwitch_norm_multilayer_2(nn.Module):
    def __init__(self):
        super(ULCSISwitch_norm_multilayer_2, self).__init__()
        self.dense1 = nn.Linear(48,24)
        self.dense2 = nn.Linear(24,12)
        self.dense3 = nn.Linear(12,1)
    def forward(self, x, slope):
        PDP = torch.squeeze(torch.sum(torch.abs(torch.fft.ifft(x[:,0,:,:]+1j*x[:,1,:,:]))**2,-2))
        max_val, _ = torch.max(PDP,-1,keepdim=True)
        PDP = PDP/max_val
        PDP_max_pool = nn.MaxPool1d(2)(PDP)
        out = self.dense1(PDP_max_pool)
        out = self.dense2(out)
        out = self.dense3(out)
        return nn.Sigmoid()(out*slope)
#        return out
class ULCSISwitch(nn.Module):
    def __init__(self):
        super(ULCSISwitch, self).__init__()
        self.dense = nn.Linear(96,1)
    def forward(self, x, slope):
        PDP = torch.squeeze(torch.sum(torch.abs(torch.fft.ifft(x[:,0,:,:]+1j*x[:,1,:,:]))**2,-2))
        max_val, _ = torch.max(PDP,-1,keepdim=True)
        PDP = PDP/max_val
...
class ULCSISwitch_norm(nn.Module):
    def __init__(self):
        super(ULCSISwitch_norm, self).__init__()
        self.dense = nn.Linear(96,1)
    def forward(self, x, slope):
        PDP = torch.squeeze(torch.sum(torch.abs(torch.fft.ifft(x[:,0,:,:]+1j*x[:,1,:,:]))**2,-2))
        norm_val = torch.norm(PDP,'fro',dim=-1,keepdim=True)
        PDP = PDP/norm_val
        out = self.dense(PDP)
        return nn.Sigmoid()(out*slope)

class ULCSISwitch_norm_2(nn.Module):
    def __init__(self):
        super(ULCSISwitch_norm_2, self).__init__()
        self.dense = nn.Linear(48,1)
    def forward(self, x, slope):
        PDP = torch.squeeze(torch.sum(torch.abs(torch.fft.ifft(x[:,0,:,:]+1j*x[:,1,:,:]))**2,-2))
        norm_val = torch.norm(PDP,p='fro',dim=-1,keepdim=True)
        PDP = PDP/norm_val
        PDP_max_pool = nn.MaxPool1d(2)(PDP)
        out = self.dense(PDP_max_pool)
        return nn.Sigmoid()(out*slope)
    
class ULCSISwitch_shift(nn.Module):
    def __init__(self):
        super(ULCSISwitch_shift, self).__init__()
        self.dense = nn.Linear(96,1)
    def forward(self, x, slope,shift):
        PDP = torch.squeeze(torch.sum(torch.abs(torch.fft.ifft(x[:,0,:,:]+1j*x[:,1,:,:]))**2,-2))
        PDP = torch.roll(PDP,shift,dims=-1)
        max_val, _ = torch.max(PDP,-1,keepdim=True)
        PDP = PDP/max_val
        out = self.dense(PDP)
        return nn.Sigmoid()(out*slope)
...
def pad_within(x, stride = 2):
  ...
  return out
def pad_within_2d(x, stride):
    # Split the input into real and imaginary parts
    x_real = torch.real(x)
    x_imag = torch.imag(x)
    
    # Create a 2D kernel for padding zeros in both dimensions
    w = x_real.new_zeros(stride[0], stride[1])  # Adjusted for 2D padding
    w[0, 0] = 1  # Only the top-left corner is 1
    
    # Apply 2D convolution transpose to both real and imaginary parts
    # We expand the kernel size to match the input channels and apply the padding
    out_real = F.conv_transpose2d(x_real, w.expand(x_real.size(1), 1, stride[0], stride[1]), 
                                  stride=[stride[0], stride[1]], groups=x_real.size(1))
    out_imag = F.conv_transpose2d(x_imag, w.expand(x_imag.size(1), 1, stride[0], stride[1]), 
                                  stride=[stride[0], stride[1]], groups=x_imag.size(1))
    
    # Combine the real and imaginary parts back
    out = out_real + 1j * out_imag
    
    return out
class SRCsiNet_BD(nn.Module):
  def __init__(self, upsample_factor):
    super(SRCsiNet_BD, self).__init__()
    self.conv1 = nn.Conv2d(2,16,kernel_size = 9, padding = 4)
    self.conv2 = nn.Conv2d(16,8,kernel_size = 1, padding = 0)
    self.conv3 = nn.Conv2d(8,1,kernel_size = 5, padding = 2) #　unused
    self.conv4 = nn.Conv2d(8,2,kernel_size = 5, padding = 2)
    
    self.conv5 = nn.Conv2d(1,8,kernel_size = [5,7], padding = [2,3])
    self.conv6 = nn.Conv2d(8,4,kernel_size = [3,5], padding = [1,2])
    self.conv7 = nn.Conv2d(4,1,kernel_size = 3, padding = 1)
  ...
  def forward(self,x, scale = 1):
    ...
    x_side = x_side[:,0,:,:] + 1j*x_side[:,1,:,:]
    x_side = torch.fft.fft(x_side,dim=-2)
    x_side_abs = torch.fft.ifft(x_side, dim=-1).abs().reshape((x_side.size(0),1,x_side.size(-2),x_side.size(-1)))
    # TODO upsampling
    out = F.relu(self.conv5(x_side_abs))
    out = F.relu(self.conv6(out))
    Mask = F.sigmoid(self.conv7(out))
    ...
    x_dl = x_dl[:,0,:,:] + 1j*x_dl[:,1,:,:]
    x_dl = x_dl.reshape((x_dl.size(0),1,x_dl.size(-2),x_dl.size(-1)))
    x_dl = pad_within_2d(x_dl,self.upsample_factor)
    x_dl = torch.fft.fft(x_dl,dim=-2)
    x_dl = torch.fft.ifft(x_dl,dim=-1).reshape((x_dl.size(0),1,x_dl.size(-2),x_dl.size(-1)))
    x_dl_abs = x_dl.abs()
    ...
    out = out[:,0,:,:] + 1j*out[:,1,:,:]
    out = torch.fft.fft(out,dim=-1).reshape((out.size(0),1,out.size(-2),out.size(-1)))
    out = torch.fft.ifft(out, dim = -2)
...
class SRCsiNet_BD_non_uniform(nn.Module):
  def __init__(self, upsample_factor):
    super(SRCsiNet_BD_non_uniform, self).__init__()
    self.conv1 = nn.Conv2d(2,16,kernel_size = 9, padding = 4)
    self.conv2 = nn.Conv2d(16,8,kernel_size = 1, padding = 0)
    self.conv3 = nn.Conv2d(8,1,kernel_size = 5, padding = 2)
    self.conv4 = nn.Conv2d(8,2,kernel_size = 5, padding = 2)
